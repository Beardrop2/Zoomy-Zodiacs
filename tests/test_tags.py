...
from bot.repositories.tags import suggest_friends


@pytest.mark.asyncio()
async def test_suggested_friends_basic_1() -> None:
    friends = [(5, "a")] * 5 + [(3, "b")] * 4
    res = await suggest_friends(friends, 1)
    assert res == {5: ["a"] * 5}
...
async def test_suggested_friends_basic_2() -> None:
    ...
    res = await suggest_friends(friends, 2)
...
async def test_suggested_friends_equal(xs: list[tuple[int, str]], amt: int) -> None:
    assume(amt > 0)
    assert await suggest_friends(xs, amt) == await suggested_friends_old(xs, amt)
